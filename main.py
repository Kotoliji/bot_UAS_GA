...
async def info_about_ga(message: types.Message):
    await message.answer(
        "ХХ Генеральна асамблея Української асоціації студентів — це подія року для студентства, яка об’єднає представників органів студентського самоврядування з усієї України.\n"
        "Це:\n\n"
        "🔸Момент єдності: цьогорічна Асамблея особлива, адже ми святкуємо 25-річчя УАС та проводимо ювілейну, ХХ Генеральну асамблею, яка об’єднає понад 200 лідерів студентських думок;\n"
        "🔹Демократичний майданчик: Генеральна асамблея є найвищим керівним органом УАС, де вирішуються ключові питання діяльності Асоціації, затверджуються стратегії та ухвалюються важливі рішення;\n"
        "🔸Місце діалогу: тут зустрінуться студентські лідери, потужні спікери, Громадські організації, щоб обговорити виклики та визначити пріоритети для розвитку студентських ініціатив;\n"
        "🔹Можливість для змін: делегати матимуть змогу обирати членів до Виконавчого комітету УАС, внести правки до Статуту Асоціації, обмінятися досвідом та з користю провести час у колі однодумців.\n\n"
        "Це не лише подія - це платформа для тих, хто прагне змінювати, творити та розвивати студентський простір. Думка студентів має силу, коли ми об'єднані та консолідовані.\n"
        "Тож давайте об’єднаємося заради спільної мети — захисту прав та інтересів студентів і побудови якісної освіти в Україні!")
...
# Хендлер для кнопки "Надіслати підписаний мандат"
@dp.message(lambda message: message.text == "Надіслати підписаний мандат")
async def register_delegates(message: types.Message, state: FSMContext):
    await state.clear()  # Очищуємо стан перед початком реєстрації
    await message.answer(
        "Мандат невдовзі буде готовий"
    )

# Хендлер для кнопки "Реєстрація студентів-відвідувачів НСФ"
@dp.message(lambda message: message.text == "Реєстрація студентів-відвідувачів НСФ")
async def register_delegates(message: types.Message, state: FSMContext):
    await state.clear()  # Очищуємо стан перед початком реєстрації
    await message.answer(
        "Скоро з'явиться"
...
# Хендлер для кнопки "Реєстрація організацій на алею ГО"
@dp.message(lambda message: message.text == "Реєстрація організацій на алею ГО")
async def register_delegates(message: types.Message, state: FSMContext):
    await state.clear()  # Очищуємо стан перед початком реєстрації
    await message.answer(
        "Незабаром буде реєстрація😊"
    )
